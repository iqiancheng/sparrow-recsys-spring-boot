...
public class SimilarMovieService {
     ...
     */
    public List<Movie> getRecList(int movieId, int size, String model) {
     ...
     */
    public List<Movie> candidateGenerator(Movie movie) {
        HashMap<Integer, Movie> candidateMap = new HashMap<>();
        for (String genre : movie.getGenres()) {
            List<Movie> oneCandidates = dataManager.getMoviesByGenre(genre, 100, "rating");
     ...
     * @return movie candidates
     */
    public List<Movie> multipleRetrievalCandidates(Movie movie) {
        if (null == movie) {
            return null;
        }
        ...
        for (String genre : genres) {
            List<Movie> oneCandidates = dataManager.getMoviesByGenre(genre, 20, "rating");
            for (Movie candidate : oneCandidates) {
                candidateMap.put(candidate.getMovieId(), candidate);
            }
        }

        List<Movie> highRatingCandidates = dataManager.getMovies(100, "rating");
        for (Movie candidate : highRatingCandidates) {
            candidateMap.put(candidate.getMovieId(), candidate);
        }

        List<Movie> latestCandidates = dataManager.getMovies(100, "releaseYear");
    ...
    public List<Movie> retrievalCandidatesByEmbedding(Movie movie, int size) {
        ...
        List<Movie> allCandidates = dataManager.getMovies(10000, "rating");
